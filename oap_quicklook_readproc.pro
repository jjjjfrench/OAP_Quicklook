FUNCTION OAP_QUICKLOOK_READPROC, STARTING_VARIABLES, FILE_VARIABLES, FIRST, LAST, Error_variable, File_run 
...
; Slicecount is read-in normally for the SPEC probes
; CIP files determine scnt using the difference between the beginning and the end of each particle
IF (prbtype EQ '2DS' or prbtype EQ 'HVPS') THEN BEGIN
  varid = NCDF_VARID(PROC, 'SliceCount')
  NCDF_VARGET, PROC, varid, scnt
ENDIF
IF (prbtype EQ 'CIP') THEN scnt = pos[1,*]-pos[0,*]
...
varid = NCDF_VARID(PROC, 'image_touching_edge')
NCDF_VARGET, PROC, varid, touching_edge
varid = NCDF_VARID(PROC, 'image_auto_reject')
NCDF_VARGET, PROC, varid, auto_reject
varid = NCDF_VARID(PROC, 'image_diam_minR')
NCDF_VARGET, PROC, varid, diam

diam = diam * 1000.  ;in file, diameter is given in mm, this changes to microns
...
RUNNING_VARIABLES=CREATE_STRUCT('HHMMSS', HHMMSS, 'START_TIME_INDEX', START_TIME_INDEX, 'END_TIME_INDEX', END_TIME_INDEX, $
  'POS', POS, 'SCNT', SCNT, 'REC', REC, 'TOUCHING_EDGE', TOUCHING_EDGE, 'AUTO_REJECT', AUTO_REJECT, 'DIAMETER', DIAM, $
  'TIME_DISP', TIME_DISP, 'POS_DISP', POS_DISP, 'PART_CNT', PART_CNT, 'DISP_PARTS', DISP_PARTS, 'DATA_LENGTH', DATA_LENGTH, $
  'DATA_WIDTH', DATA_WIDTH, 'TMP_LENGTH', TMP_LENGTH, 'TMP_WIDTH', TMP_WIDTH)
